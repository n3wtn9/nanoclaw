...
Only the authentication variables (`CLAUDE_CODE_OAUTH_TOKEN` and `ANTHROPIC_API_KEY`) are extracted from `.env` and passed to the container via stdin JSON. This ensures other environment variables in `.env` are not exposed to the agent.
...
| "Claude Code process exited with code 1" | Docker not running | Check logs; ensure Docker is started |
